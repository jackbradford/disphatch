...
class Config {
	...
	private function defineGlobalConstants() {
		...
		$templates		=	$this->getDirective('templates');
		$defaultActn	=	$this->getDirective('default_action');
		$ctrl_qs		=	$this->getDirective('ctrl_query_str');
		$act_qs			=	$this->getDirective('action_query_srt');
		$asyncFlag		=	$this->getDirective('async_post_flag');
		$defaultCtrl	=	$controllers->default;
		unset($controllers->default);

		define('CONTROLLERS',			(array) $controllers);
		define('TEMPLATES',				(array) $templates);
		define('DEFAULT_CONTROLLER',	$defaultCtrl);
		define('DEFAULT_ACTION',		$defaultActn);
		define('CTRL_QUERY_STR',		$ctrl_qs);
		define('ACTION_QUERY_STR',		$act_qs);
		define('ASYNC_POST_FLAG',		$asyncFlag);
	}
